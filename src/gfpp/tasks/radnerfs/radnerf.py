...
from gfpp.utils.nn.schedulers import ExponentialScheduleForRADNeRF
...
class RADNeRFTask(BaseTask):
    ...
    def run_model(self, sample, infer=False):
        ...
        start_finetune_lip = (
            hparams["finetune_lips"]
            and self.global_step > hparams["finetune_lips_start_iter"]
        )
        ...
        if not infer:
            ...
            if self.model.training:
                ...
                if start_finetune_lip and self.finetune_lip_flag:
                    ...
                    gt_rgb = (
                        gt_rgb.view(-1, xmax - xmin, ymax - ymin, 3)
                        .permute(0, 3, 1, 2)
                        .contiguous()
                    )
                    pred_rgb = (
                    ...
                    losses_out["lpips_loss"] = self.criterion_lpips(
                        pred_rgb, gt_rgb
                    ).mean()
            else:
                ...
                if "lip_rect" in sample:
                    xmin, xmax, ymin, ymax = sample["lip_rect"]
                    lip_gt_rgb = (
                        gt_rgb.view(-1, H, W, 3)[:, xmin:xmax, ymin:ymax, :]
                        .permute(0, 3, 1, 2)
                        .contiguous()
                    )
                    lip_pred_rgb = (
                        pred_rgb.view(-1, H, W, 3)[:, xmin:xmax, ymin:ymax, :]
                        .permute(0, 3, 1, 2)
                        .contiguous()
                    )
                    losses_out["lpips_loss"] = self.criterion_lpips(
                        lip_pred_rgb, lip_gt_rgb
                    ).mean()
            ...
            if "gt_img" in sample:
                ...
                if "lip_rect" in sample:
                    ...
                    model_out["lpips_loss"] = self.criterion_lpips(
                        pred_rgb, gt_rgb
                    ).mean()
            return model_out
    ...
    def _training_step(self, sample, batch_idx, optimizer_idx):
        ...
        loss_weights = {
            "lap_ambient_loss": hparams.get("lambda_lap_ambient_loss", 0.0),
            "mse_loss": 1.0,
            "weights_entropy_loss": hparams["lambda_weights_entropy"],
            "lpips_loss": hparams["lambda_lpips_loss"],
            "ambient_loss": min(self.global_step / 250000, 1.0)
            * hparams["lambda_ambient"],  # gradually increase it
        }
        total_loss = sum(
            [
