...
from gfpp.tasks.radnerfs.radnerf_sr import RADNeRFTask
...
from gfpp.utils.nn.model_utils import (
    print_arch,
    num_params,
    not_requires_grad,
    requires_grad,
...
from gfpp.tasks.radnerfs.radnerf_sr import FeatureMatchingLoss
...
class RADNeRFTorsoTask(RADNeRFTask):
    ...
    def build_model(self):
        ...
        self.torso_embedders_params = [
            p
            for k, p in self.model.named_parameters()
            if p.requires_grad and "torso_embedder" in k
        ]
        self.torso_network_params = [
            p
            for k, p in self.model.named_parameters()
            if (p.requires_grad and "torso_embedder" not in k and "torso" in k)
        ]
        self.sr_net_params = [
            p for k, p in self.model.sr_net.named_parameters() if p.requires_grad
        ]
    ...
    def build_scheduler(self, optimizer):
        return ExponentialScheduleForRADNeRFTorso(
    ...
    def run_model(self, sample, infer=False):
        ...
        if not infer:
            ...
            losses_out["torso_mse_loss"] = torch.mean(
                (pred_torso_rgb - gt_torso_rgb) ** 2
            )  # [B, N, 3] -->  scalar
            alphas = model_out["torso_alpha_map"].clamp(1e-5, 1 - 1e-5)
            losses_out["torso_weights_entropy_loss"] = torch.mean(
                -alphas * torch.log2(alphas) - (1 - alphas) * torch.log2(1 - alphas)
            )  # you can visualize this fn at https://www.desmos.com/calculator/rwbs7bruvj?lang=zh-TW
            ...
            if self.global_step >= hparams["sr_start_iters"]:
                ...
                gt_rgb_512 = (
                    sample["gt_img_512"].reshape([1, 512, 512, 3]).permute(0, 3, 1, 2)
                )
                gt_torso_rgb_512 = (
                    sample["bg_torso_img_512"]
                    .reshape([1, 512, 512, 3])
                    .permute(0, 3, 1, 2)
                )
                losses_out["sr_torso_mse_loss"] = torch.mean(
                    (sr_pred_torso_rgb - gt_torso_rgb_512) ** 2
                )  # [B, N, 3] -->  scalar

            if self.global_step >= hparams["lpips_start_iters"]:
                losses_out["torso_lpips_loss"] = self.criterion_lpips(
                    pred_torso_rgb, gt_torso_rgb
                ).mean()
                losses_out["sr_torso_lpips_loss"] = self.criterion_lpips(
                    sr_pred_torso_rgb, gt_torso_rgb_512
                ).mean()
    ...
    def _training_step(self, sample, batch_idx, optimizer_idx):
        ...
        loss_weights = {
            ...
            "torso_deform_loss": (
                hparams["lambda_torso_deform"] if self.global_step >= 10_0000 else 0
            ),
            "sr_torso_mse_loss": (
                1.0 if self.global_step >= hparams["sr_start_iters"] else 0
            ),
            "torso_lpips_loss": (
        ...
        def mse2psnr(x):
            return (
                -10.0
                * torch.log(x + 1e-10)
                / torch.log(torch.Tensor([10.0])).to(x.device)
            )
        ...
        if (self.global_step + 1) % hparams["tb_log_interval"] == 0:
            density_grid_info = {
                ...
                "density_grid_info/occupancy_rate_torso": (
                    self.model.density_grid_torso
                    > min(
                        self.model.mean_density_torso, self.model.density_thresh_torso
                    )
                ).sum()
    ...
    def on_before_optimization(self, opt_idx):
        ...
        grad_norm_dict = {
            f"{prefix}/torso_embedders_params": get_grad_norm(
                self.torso_embedders_params
            ),
