...
from gfpp.modules.radnerfs.utils import convert_poses, get_bg_coords, get_rays
from gfpp.tasks.radnerfs.losses import PerceptualLoss
from gfpp.modules.eg3ds.models.dual_discriminator import DualDiscriminator
from gfpp.tasks.radnerfs.dataset_utils import RADNeRFDataset

from gfpp.utils.commons.image_utils import to8b
from gfpp.utils.commons.base_task import BaseTask
from gfpp.utils.commons.dataset_utils import data_loader
from gfpp.utils.commons.hparams import hparams
...
from gfpp.utils.nn.schedulers import NoneSchedule
from gfpp.utils.nn.grad import get_grad_norm
...
class FeatureMatchingLoss(nn.Module):
    ...
    def forward(self, fake_features, real_features):
        ...
        for i in range(num_feature_maps):
            ...
            if (
                hw < 32
            ):  # following VGG19, we don't compute on small feature maps, see modules.os_avatar.facev2v_warp.losses.PerceptualLoss
                continue
...
class RADNeRFTask(BaseTask):
    ...
    def build_model(self):
        ...
        self.embedders_params += [
            ...
            if p.requires_grad and "position_embedder" in k
        ]
        self.embedders_params += [
            p
            for k, p in self.model.named_parameters()
            if p.requires_grad and "ambient_embedder" in k
        ]
        self.network_params = [
            p
            for k, p in self.model.named_parameters()
            if (
                ...
                and "ambient_embedder" not in k
                and "cond_att_net" not in k
            )
        ]
        self.att_net_params = [
            p
            for k, p in self.model.named_parameters()
            if p.requires_grad and "cond_att_net" in k
        ]
        # sr_net also belongs to the newtwork_params

        self.model.conds = self.train_dataset.conds
        self.model.mark_untrained_grid(
            self.train_dataset.poses, self.train_dataset.intrinsics
        )
    ...
    def build_scheduler(self, optimizer):
        return ExponentialScheduleForRADNeRF(
            optimizer, hparams["lr"], hparams["warmup_updates"]
        )
    ...
    def run_model(self, sample, infer=False):
        ...
        bg_color = (
            sample["bg_torso_img"] if "bg_torso_img" in sample else sample["bg_img"]
        )  # treat torso as a part of background
        ...
        if not infer:
            ...
            model_out = self.model.render(
                ...
                cond_mask=cond_mask,
                eye_area_percent=eye_area_percent,
                **hparams,
            )
            ...
            # loss on img_raw
            losses_out["mse_loss"] = torch.mean(
                (pred_rgb - gt_rgb) ** 2
            )  # [B, N, 3] -->  scalar
            ...
            if self.model.training:
                ...
                if hparams.get("ambient_loss_mode", "mae") == "mae":
                    losses_out["ambient_loss"] = (
                        ambient.abs() * (~face_mask.view(-1))
                    ).sum() / ((~face_mask).sum() + 1)
                else:
                    losses_out["ambient_loss"] = (
                        ambient.pow(2) * (~face_mask.view(-1))
                    ).sum() / ((~face_mask).sum() + 1)
            ...
            if self.global_step >= hparams["sr_start_iters"]:
                ...
                losses_out["sr_mse_loss"] = torch.mean(
                    (sr_pred_rgb - gt_rgb_512) ** 2
                )  # [B, N, 3] -->  scalar

            if self.global_step >= hparams["lpips_start_iters"]:
                losses_out["lpips_loss"] = self.criterion_lpips(pred_rgb, gt_rgb).mean()
                losses_out["sr_lpips_loss"] = self.criterion_lpips(
                    sr_pred_rgb, gt_rgb_512
                ).mean()
                xmin, xmax, ymin, ymax = sample[
                    "lip_rect"
                ]  # in 256 resolution so need to x2
                losses_out["sr_lip_lpips_loss"] = self.criterion_lpips(
                    sr_pred_rgb[:, :, xmin * 2 : xmax * 2, ymin * 2 : ymax * 2],
                    gt_rgb_512[:, :, xmin * 2 : xmax * 2, ymin * 2 : ymax * 2],
                ).mean()

            if (
                self.global_step >= hparams["lpips_start_iters"]
                and hparams["lambda_dual_fm"] > 0
            ):
                ...
                with torch.cuda.amp.autocast(False):  # Disc有部分不支持amp
                    fake_logit = self.dual_disc(
                        fake_image, camera, feature_maps=fake_feature_maps
                    )
                    with torch.no_grad():
                        real_logit = self.dual_disc(
                            real_image, camera, feature_maps=real_feature_maps
                        )
                losses_out["dual_feature_matching_loss"] = (
                    self.dual_feature_matching_loss_fn(
                        fake_feature_maps, real_feature_maps
                    )
                )
    ...
    def _training_step(self, sample, batch_idx, optimizer_idx):
        ...
        loss_weights = {
            ...
            "dual_feature_matching_loss": (
                hparams["lambda_dual_fm"]
                if self.global_step >= hparams["lpips_start_iters"]
                else 0
            ),
        }
        ...
        def mse2psnr(x):
            return -10.0 * torch.log(x) / torch.log(torch.Tensor([10.0])).to(x.device)
        ...
        try:
            ...
            grad_lambda_ambient = math.log10(current_ambient_loss + 1e-15) - math.log10(
                target_ambient_loss + 1e-15
            )  # 如果需要增大lambda_ambient， 则current_loss大于targt，grad值大于0
            lr_lambda_ambient = hparams["lr_lambda_ambient"]
            self.model.lambda_ambient.data = (
                self.model.lambda_ambient.data + grad_lambda_ambient * lr_lambda_ambient
            )
        ...
        if (self.global_step + 1) % hparams["tb_log_interval"] == 0:
            density_grid_info = {
                ...
                "density_grid_info/occupancy_rate": (
                    self.model.density_grid
                    > min(self.model.mean_density, self.model.density_thresh)
                ).sum()
    ...
    def validation_step(self, sample, batch_idx):
        ...
        if (
            ...
            if torch.distributed.is_initialized():
                sample = move_to_cuda(
                    self.val_dataset[idx_lst[batch_idx]],
            ...
            img_pred_sr = (
                infer_outputs["sr_rgb_map"].permute(0, 2, 3, 1).reshape([512, 512, 3])
            )
            depth_pred = infer_outputs["depth_map"].reshape([H, W])

            base_fn = f"frame_{sample['idx']}"
            self.logger.add_figure(
                f"frame_{sample['idx']}/img_pred",
                self.rgb_to_figure(img_pred),
                self.global_step,
            )
            self.logger.add_figure(
                f"frame_{sample['idx']}/depth_pred",
                self.rgb_to_figure(depth_pred),
                self.global_step,
            )
            ...
            if hparams["save_gt"]:
                ...
                img_gt_512 = (
                    sample["gt_img_512"].permute(0, 2, 3, 1).reshape([512, 512, 3])
                )
                if self.global_step == hparams["valid_infer_interval"]:
                    self.logger.add_figure(
                        f"frame_{sample['idx']}/img_gt",
                        self.rgb_to_figure(img_gt),
                        self.global_step,
                    )
                ...
                self.save_rgb_to_fname(
                    img_gt_512, f"{self.gen_dir}/images/{base_fn}.png"
                )
    ...
    def rgb_to_figure(rgb):
        ...
        rgb = convert_to_np(rgb * 255.0).astype(np.uint8)
        plt.imshow(rgb)
        return fig
    ...
    ### GUI utils
    def test_gui_with_editable_data(
        self,
        pose,
        intrinsics,
        W,
    ...
    def test_gui_with_data(self, sample, target_W, target_H):
        ...
        preds = (
            F.interpolate(
                preds.permute(0, 3, 1, 2), size=(target_H, target_W), mode="bilinear"
            )
            .permute(0, 2, 3, 1)
            .contiguous()
        )
        preds_depth = F.interpolate(
            preds_depth.unsqueeze(1), size=(target_H, target_W), mode="nearest"
        ).squeeze(1)
