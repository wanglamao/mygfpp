...
from gfpp.utils.commons.tensor_utils import convert_to_tensor, convert_to_np
from gfpp.utils.commons.image_utils import load_image_as_uint8_tensor

# from gfpp.utils.commons.meters import Timer
...
def get_lf_boundary_mask(lm468, h=512, w=512, index_mode="lm468"):
    ...
    if index_mode == "lm468":
        ...
        index_lf_from_lm68 = (
            [4, 5, 6, 7, 8, 9, 10, 11, 12] + list(range(27, 36)) + list(range(48, 68))
        )
...
def get_ldm_heatmap(spatial_size, ldm, std=0.006):
    ...
    ldm_normed = ldm / torch.FloatTensor(spatial_size)[None, :].to(
        ldm.device
    )  # [N_kp, 2]
    heatmap = torch.exp(
        -(
            (meshed[None, None, :, :, None, :] - ldm_normed[:, :, None, None, :, :])
            ** 2
        ).sum(-1)
        / 2
        / (std**2)
...
class RADNeRFDataset(torch.utils.data.Dataset):
    def __init__(self, prefix, data_dir=None, training=True):
        ...
        self.data_dir = (
            os.path.join(
                "/mnt/nas/avatars/mygfpp/data/binary/videos",
                str(hparams["video_id"]),  # fuck
            )
            # os.path.join(hparams["binary_data_dir"], str(hparams["video_id"]))
            if data_dir is None
            else str(data_dir)
        )
        ...
        if prefix == "train":
            self.samples = [
                convert_to_tensor(sample) for sample in ds_dict["train_samples"]
            ]
        elif prefix == "val":
            self.samples = [
        ...
        if hparams["infer_bg_img_fname"] == "":
            ...
            bg_img = torch.from_numpy(ds_dict["bg_img"]).float() / 255.0
            self.bg_img_512 = convert_to_tensor(bg_img).cuda()
            bg_img = (
                F.interpolate(
                    bg_img.unsqueeze(0).permute(0, 3, 1, 2),
                    mode="bilinear",
                    size=(self.H, self.W),
                    antialias=True,
                )
            ...
            bg_img = cv2.imread(
                hparams["infer_bg_img_fname"], cv2.IMREAD_UNCHANGED
            )  # [H, W, 3]
        ...
        if not training and hparams["infer_smooth_camera_path"]:
            ...
            smo_c2w = smooth_camera_path(
                c2w_arr.numpy(),
                kernel_size=hparams["infer_smooth_camera_path_kernel_size"],
            )
        ...
        self.poses = torch.from_numpy(
            np.stack(
                [
                    nerf_matrix_to_ngp(
                        s["c2w"],
                        scale=hparams["camera_scale"],
                        offset=hparams["camera_offset"],
                    )
        ...
        if torch.any(torch.isnan(self.poses)):
            raise ValueError(
                "Found NaN in transform_matrix, please check the face_tracker process!"
            )
        ...
        if self.cond_type == "deepspeech":
            ...
            id, exp = convert_to_tensor(ds_dict["id"]), convert_to_tensor(
                ds_dict["exp"]
            )
            ...
            euler, trans = convert_to_tensor(ds_dict["euler"]), convert_to_tensor(
                ds_dict["trans"]
            )
            ...
            eg3d_camera_ret = get_eg3d_convention_camera_pose_intrinsic(
                {"euler": euler, "trans": trans}
            )
            self.eg3d_cameras = convert_to_tensor(
                np.concatenate(
                    [
                        eg3d_camera_ret["c2w"].reshape([-1, 16]),
                        eg3d_camera_ret["intrinsics"].reshape([-1, 9]),
                    ],
                    axis=-1,
                )
            )

            self.keypoint_mode = keypoint_mode = hparams.get(
            ...
            idexp_lm3d_normalized_win = idexp_lm3d_normalized.reshape(
                [-1, 1, self.keypoint_num * 3]
            )
        ...
        if hparams.get("with_sr"):
            self.lips_rect = (
                (np.array([s["lip_rect"] for s in self.samples]) / 2)
                .astype(int)
                .tolist()
            )
    ...
    def __getitem__(self, idx):
        ...
        if self.hparams.get("load_imgs_to_memory", True):
            ...
            if "torso_img" not in self.samples[idx].keys():
                self.samples[idx]["torso_img"] = load_image_as_uint8_tensor(
                    self.samples[idx]["torso_img_fname"]
                )
                self.samples[idx]["gt_img"] = load_image_as_uint8_tensor(
                    self.samples[idx]["gt_img_fname"]
                )
        ...
        sample["gt_img_512"] = (
            gt_img.cuda().unsqueeze(0).permute(0, 3, 1, 2) / 255.0
        )  # b,c,h,w

        sample["cond_wins"] = get_audio_features(self.conds, att_mode=2, index=idx)
        sample["cond_wins_prev"] = get_audio_features(
            self.conds, att_mode=2, index=max(idx - 1, 0)
        )
        sample["cond_wins_next"] = get_audio_features(
            self.conds, att_mode=2, index=min(idx + 1, len(self) - 1)
        )
        ...
        sample.update(
            {
                "torso_img": torso_img.cuda().float() / 255.0,
                "gt_img": gt_img.cuda().float() / 255.0,
            }
        )
        ...
        if hparams.get("with_sr"):
            ...
            rays = get_rays(
                ngp_pose.cuda(), self.intrinsics, self.H, self.W, N=-1, rect=None
            )
        else:
            # 否则,沿用adnerf的策略
            if self.training:
        ...
        if hparams.get("polygon_face_mask", True):
            f_mask = dilate_boundary_mask(
                get_boundary_mask(
                    self.lm2ds[idx], index_mode="lm68", h=self.H, w=self.W
                )
                .unsqueeze(0)
                .cuda(),
                ksize=3,
        ...
        sample["cond_mask"] = face_mask.reshape(
            [
                -1,
            ]
        )
        ...
        if hparams.get("with_sr"):
            bg_torso_img = F.interpolate(
                bg_torso_img.cuda().view(1, 512, 512, -1).permute(0, 3, 1, 2),
                size=(self.H, self.W),
                mode="bilinear",
                antialias=True,
            ).permute(
            ...
            gt_img = (
                F.interpolate(
                    gt_img.view(1, 512, 512, 3).permute(0, 3, 1, 2),
                    size=(self.H, self.W),
                    mode="bilinear",
                    antialias=True,
                )
        ...
        bg_torso_img = bg_torso_img.view(
            1, -1, 3
        )  # treat torso as a part of background
        bg_img = self.bg_img.view(1, -1, 3)

        bg_torso_img_512 = bg_torso_img_512[..., :3] * bg_torso_img_512[
            ..., 3:
        ] + self.bg_img_512 * (1 - bg_torso_img_512[..., 3:])
        bg_torso_img_512 = bg_torso_img_512.view(
            1, -1, 3
        )  # treat torso as a part of background
        ...
        # if self.training:
        bg_img = torch.gather(
            bg_img.cuda(), 1, torch.stack(3 * [rays["inds"]], -1)
        )  # [B, N, 3]
        bg_torso_img = torch.gather(
            bg_torso_img.cuda(), 1, torch.stack(3 * [rays["inds"]], -1)
        )  # [B, N, 3]
        gt_img = torch.gather(
            gt_img.reshape(1, -1, C).cuda(), 1, torch.stack(C * [rays["inds"]], -1)
        )  # [B, N, 3/4]
        ...
        if self.training:
            bg_coords = torch.gather(
                self.bg_coords.cuda(), 1, torch.stack(2 * [rays["inds"]], -1)
            )  # [1, N, 2]
        else:
