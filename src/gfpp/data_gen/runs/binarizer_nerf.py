...
from gfpp.utils.commons.euler2rot import euler_trans_2_c2w, c2w_to_euler_trans
from gfpp.data_gen.utils.process_video.euler2quaterion import (
    euler2quaterion,
    quaterion2euler,
)
...
def rot2euler(rot_mat):
    ...
    cos_y = torch.sqrt(
        rot_mat[:, 1, 2] * rot_mat[:, 1, 2] + rot_mat[:, 2, 2] * rot_mat[:, 2, 2]
    )
...
def get_win_conds(conds, idx, smo_win_size=8, pad_option="zero"):
    ...
    if pad_left > 0:
        if pad_option == "zero":
            conds_win = np.concatenate(
                [np.zeros_like(conds_win)[:pad_left], conds_win], axis=0
            )
    ...
    if pad_right > 0:
        if pad_option == "zero":
            conds_win = np.concatenate(
                [conds_win, np.zeros_like(conds_win)[:pad_right]], axis=0
            )
        elif pad_option == "edge":
...
def load_processed_data(processed_dir):
    ...
    ret_dict["focal"], ret_dict["cx"], ret_dict["cy"] = (
        face_model.focal,
        face_model.center,
        face_model.center,
    )
    ...
    eye_area_percent = get_eye_area_percent(
        torch.tensor(coeff_dict["id"]), torch.tensor(coeff_dict["exp"]), face3d_helper
    )
    ...
    idexp_lm3d_arr = (
        face3d_helper.reconstruct_idexp_lm3d(
            torch.from_numpy(identity_arr), torch.from_numpy(exp_arr)
        )
        .cpu()
        .numpy()
        .reshape([-1, 68 * 3])
    ...
    rots = face_model.compute_rotation(
        eulers
    )  # rotation matrix is a better intermediate for convention-transplan than euler

    # handle the camera pose to geneface's convention
    trans[:, 2] = (
        10 - trans[:, 2]
    )  # 抵消fit阶段的to_camera操作，即trans[...,2] = 10 - trans[...,2]
    ...
    pose = (
        torch.eye(4, dtype=torch.float32).unsqueeze(0).repeat([len_motion, 1, 1])
    )  # [T, 4, 4]
    pose[:, :3, :3] = rots_inv
...
if __name__ == "__main__":
    ...
    face_model = ParametricFaceModel(
        bfm_folder="deep_3drecon/BFM", camera_distance=10, focal=1015
    )
    face_model.to("cpu")
