...
from gfpp.utils.commons.multiprocess_utils import (
...
def scatter_np(condition_img, classSeg=5):
    ...
    input_label = np.zeros(
        [batch, classSeg, condition_img.shape[2], condition_img.shape[3]]
    ).astype(np.int_)
...
def scatter(condition_img, classSeg=19):
    ...
    input_label = torch.zeros(
        batch,
        classSeg,
        condition_img.shape[2],
        condition_img.shape[3],
...
def decode_segmap_mask_from_segmap_video_frame(video_frame):
    ...
    def assign_values(array):
        ...
        assigned_values = np.where(
            remainder <= 20, array - remainder, array + (40 - remainder)
        )
...
def job_cal_seg_map_for_image(img, segmenter_options=None, segmenter=None):
    ...
    if segmenter is not None:
        ...
        global_segmenter = (
            vision.ImageSegmenter.create_from_options(segmenter_options)
            if global_segmenter is None
            else global_segmenter
        )
...
class MediapipeSegmenter:
    ...
    def _cal_seg_map_for_video(
        self, imgs, segmenter=None, return_onehot_mask=True, return_segmap_image=True
    ):
        segmenter = (
            vision.ImageSegmenter.create_from_options(self.video_options)
            if segmenter is None
            else segmenter
        )
        assert (
            return_onehot_mask or return_segmap_image
        )  # you should at least return one
        segmap_masks = []
        segmap_images = []
        for i in tqdm.trange(len(imgs), desc="extracting segmaps from a video..."):
            ...
            if return_onehot_mask:
                segmap_mask = scatter_np(segmap[None, None, ...], classSeg=6)[
                    0
                ]  # [6, H, W]
                segmap_masks.append(segmap_mask)
    ...
    def _cal_seg_map(self, img, segmenter=None, return_onehot_mask=True):
        ...
        assert img.ndim == 3
        segmenter = (
            vision.ImageSegmenter.create_from_options(self.options)
            if segmenter is None
            else segmenter
        )
    ...
    def _seg_out_img_with_segmap(self, img, segmap, mode="head"):
        ...
        if mode == "head":
            selected_mask = (
                segmap[[1, 3, 5], :, :].sum(axis=0)[None, :] > 0.5
            )  # glasses 也属于others
            img[~selected_mask.repeat(3, axis=0).transpose(1, 2, 0)] = (
                0  # (-1,-1,-1) denotes black in our [-1,1] convention
            )
            ...
            selected_mask = (
                segmap[[0], :, :].sum(axis=0)[None, :] > 0.5
            )  # only seg out 0, which means background
            img[~selected_mask.repeat(3, axis=0).transpose(1, 2, 0)] = (
                0  # (-1,-1,-1) denotes black in our [-1,1] convention
            )
        elif mode == "full":
    ...
    def _seg_out_img(self, img, segmenter=None, mode="head"):
        ...
        segmap = self._cal_seg_map(
            img, segmenter=segmenter, return_onehot_mask=True
        )  # [B, 19, H, W]
