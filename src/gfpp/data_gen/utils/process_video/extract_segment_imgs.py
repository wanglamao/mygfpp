...
from gfpp.utils.commons.tensor_utils import convert_to_np
from gfpp.utils.commons.os_utils import multiprocess_glob
import pickle
import traceback
import multiprocessing
from gfpp.utils.commons.multiprocess_utils import multiprocess_run_tqdm
from scipy.ndimage import binary_erosion, binary_dilation
from sklearn.neighbors import NearestNeighbors
from mediapipe.tasks.python import vision
from gfpp.data_gen.utils.mp_feature_extractors.mp_segmenter import (
    MediapipeSegmenter,
    encode_segmap_mask_to_image,
    decode_segmap_mask_from_image,
    job_cal_seg_map_for_image,
)
...
from gfpp.data_gen.utils.process_video.split_video_to_imgs import extract_img_job
...
def save_rgb_alpha_image_to_path(img, alpha, img_path):
    try:
        ...
        pass
    cv2.imwrite(
        img_path, np.concatenate([cv2.cvtColor(img, cv2.COLOR_RGB2BGR), alpha], axis=-1)
    )
...
def extract_background(
    img_lst, segmap_mask_lst=None, method="knn", device="cpu", mix_bg=True
):
    """
    ...
    def get_segmap_mask(img_lst, segmap_mask_lst, index):
        if segmap_mask_lst is not None:
            ...
            segmap = seg_model._cal_seg_map(
                refresh_image(img_lst[index]), segmenter=segmenter
            )
        return segmap
    ...
    if method == "knn":
        ...
        img_lst = (
            img_lst[::FRAME_SELECT_INTERVAL]
            if num_frames > FRAME_SELECT_INTERVAL
            else img_lst[0:1]
        )

        if segmap_mask_lst is not None:
            segmap_mask_lst = (
                segmap_mask_lst[::FRAME_SELECT_INTERVAL]
                if num_frames > FRAME_SELECT_INTERVAL
                else segmap_mask_lst[0:1]
            )
        ...
        # nearest neighbors
        all_xys = (
            np.mgrid[0:h, 0:w].reshape(2, -1).transpose()
        )  # [512*512, 2] coordinate grid
        distss = []
        for idx, img in tqdm.tqdm(
            enumerate(img_lst), desc="combining backgrounds...", total=len(img_lst)
        ):
            segmap = get_segmap_mask(
                img_lst=img_lst, segmap_mask_lst=segmap_mask_lst, index=idx
            )
            bg = (segmap[0]).astype(bool)  # [h,w] bool mask
            fg_xys = np.stack(np.nonzero(~bg)).transpose(
                1, 0
            )  # [N_nonbg,2] coordinate of non-bg pixels
            nbrs = NearestNeighbors(n_neighbors=1, algorithm="kd_tree").fit(fg_xys)
            dists, _ = nbrs.kneighbors(
                all_xys
            )  # [512*512, 1] distance to nearest non-bg pixel
            distss.append(dists)
        ...
        bc_pixs = (
            max_dist > 10
        )  # 在各个frame有一个出现过是bg的pixel，bg标准是离最近的non-bg pixel距离大于10
        ...
        bg_img[bc_pixs_id, :] = imgs[
            bc_ids, bc_pixs_id, :
        ]  # 对那些铁bg的pixel，直接去对应的image里面采样
        ...
        distances, indices = nbrs.kneighbors(
            bg_xys
        )  # 对non-bg img，用KNN找最近的bg pixel
        bg_fg_xys = fg_xys[indices[:, 0]]
        bg_img[bg_xys[:, 0], bg_xys[:, 1], :] = bg_img[
            bg_fg_xys[:, 0], bg_fg_xys[:, 1], :
        ]
    else:
...
def generate_segment_imgs_job(img_name, segmap, img):
    out_img_name = segmap_name = img_name.replace("/gt_imgs/", "/segmaps/").replace(
        ".jpg", ".png"
    )  # 存成jpg的话，pixel value会有误差
    try:
    ...
    for mode in ["head", "torso", "person", "bg"]:
        ...
        img_alpha = 255 * np.ones(
            (img.shape[0], img.shape[1], 1), dtype=np.uint8
        )  # alpha
        mask = mask[0][..., None]
        img_alpha[~mask] = 0
        out_img_name = img_name.replace("/gt_imgs/", f"/{mode}_imgs/").replace(
            ".jpg", ".png"
        )
    ...
    (
        inpaint_torso_img,
        inpaint_torso_img_mask,
        inpaint_torso_with_bg_img,
        inpaint_torso_with_bg_img_mask,
    ) = inpaint_torso_job(img, segmap)
    img_alpha = 255 * np.ones((img.shape[0], img.shape[1], 1), dtype=np.uint8)  # alpha
    img_alpha[~inpaint_torso_img_mask[..., None]] = 0
    out_img_name = img_name.replace("/gt_imgs/", f"/inpaint_torso_imgs/").replace(
        ".jpg", ".png"
    )
...
def segment_and_generate_for_image_job(
    img_name, img, segmenter_options=None, segmenter=None, store_in_memory=False
):
    img = refresh_image(img)
    segmap_mask, segmap_image = job_cal_seg_map_for_image(
        img, segmenter_options=segmenter_options, segmenter=segmenter
    )
    segmap_name = generate_segment_imgs_job(
        img_name=img_name, segmap=segmap_mask, img=img
    )
    if store_in_memory:
        return segmap_mask
    else:
...
def extract_segment_job(
    ...
    try:
        ...
        if nerf:  # single video
            raw_img_dir = video_name.replace(".mp4", "/gt_imgs/").replace(
                "/raw/", "/processed/"
            )
        else:  # whole dataset
        ...
        if not os.path.exists(raw_img_dir):
            extract_img_job(
                video_name, raw_img_dir
            )  # use ffmpeg to split video into imgs
        ...
        if multiprocess_enable:
            ...
            for index in tqdm.tqdm(
                range(len(img_lst)),
                desc="generating segment images in single-process...",
            ):
        ...
        if nerf:
            out_img_name = video_name.replace("/raw/", "/processed/").replace(
                ".mp4", f"/{bg_prefix_name}.jpg"
            )
        else:
            out_img_name = video_name.replace(
                "/video/", f"/{bg_prefix_name}_img/"
            ).replace(".mp4", ".jpg")
...
def out_exist_job(vid_name, background_method="knn"):
    ...
    if (
        ...
        and os.path.exists(out_dir1)
        and os.path.exists(out_dir1)
        and os.path.exists(out_dir2)
    ):
        num_frames = len(os.listdir(img_dir))
        if (
            len(os.listdir(out_dir1)) == num_frames
            and len(os.listdir(out_dir2)) == num_frames
        ):
            return None
        else:
            return vid_name
...
def get_todo_vid_names(vid_names, background_method="knn"):
    ...
    for i, res in multiprocess_run_tqdm(
        out_exist_job, fn_args, num_workers=16, desc="checking todo videos..."
    ):
        if res is not None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--background_method",
        choices=["knn", "mat", "ddnm", "lama"],
        type=str,
        default="knn",
    )
    parser.add_argument(
        "--total_gpus", default=0, type=int
    )  # zero gpus means utilizing cpu
    parser.add_argument("--no_mix_bg", action="store_true")
    parser.add_argument(
        "--store_in_memory", action="store_true"
    )  # set to True to speed up preprocess, but leads to high memory costs
    parser.add_argument(
        "--force_single_process", action="store_true"
    )  # turn this on if you find multi-process does not work on your environment
    ...
    if ds_name.lower() == "nerf":  # 处理单个视频
        vid_names = [vid_dir]
        out_names = [
            video_name.replace("/raw/", "/processed/").replace(".mp4", "_lms.npy")
    ...
    if total_process > 1:
        ...
        if process_id == total_process:
            ...
            vid_names = vid_names[
                process_id
                * num_samples_per_process : (process_id + 1)
    ...
    if ds_name == "nerf":  # 处理单个视频
        ...
        for vid_name in multiprocess_run_tqdm(
            extract_job,
            fn_args,
