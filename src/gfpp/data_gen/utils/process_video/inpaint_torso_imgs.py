...
from gfpp.tasks.eg3ds.loss_utils.segment_loss.mp_segmenter import MediapipeSegmenter
...
def inpaint_torso_job(video_name, idx=None, total=None):
    ...
    for image_path in tqdm.tqdm(img_names):
        ...
        head_image = cv2.imread(
            image_path.replace("/gt_imgs/", "/head_imgs/"), cv2.IMREAD_UNCHANGED
        )  # [H, W, 3]
        torso_image = cv2.imread(
            image_path.replace("/gt_imgs/", "/torso_imgs/"), cv2.IMREAD_UNCHANGED
        )  # [H, W, 3]
        bg_image = cv2.imread(
            image_path.replace("/gt_imgs/", "/bg_imgs/"), cv2.IMREAD_UNCHANGED
        )  # [H, W, 3]
...
def out_exist_job(vid_name):
    ...
    out_dir2 = vid_name.replace("/video/", "/inpaint_torso_with_bg_imgs/").replace(
        ".mp4", ""
    )
