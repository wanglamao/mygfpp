...
from gfpp.utils.commons.tensor_utils import convert_to_tensor
...
def polygon_area(x, y):
    ...
    main_area = (x_[:, :-1] * y_[:, 1:]).sum(axis=-1) - (y_[:, :-1] * x_[:, 1:]).sum(
        axis=-1
    )
...
if __name__ == "__main__":
    ...
    import cv2
    import torch
    from gfpp.data_gen.utils.process_video.extract_lm2d import (
        extract_lms_mediapipe_job,
        read_video_to_frames,
        index_lm68_from_lm468,
    )
    from gfpp.data_gen.utils.process_video.fit_3dmm_landmark import fit_3dmm_for_a_video
    from data_util.face3d_helper import Face3DHelper
    ...
    area_percent = get_eye_area_percent(
        torch.tensor(coeff["id"]), torch.tensor(coeff["exp"]), face3d_helper
    )
    writer = imageio.get_writer("1.mp4", fps=25)
    for idx, frame in enumerate(frames):
        frame = cv2.putText(
            frame,
            f"{area_percent[idx]:.2f}",
            org=(128, 128),
