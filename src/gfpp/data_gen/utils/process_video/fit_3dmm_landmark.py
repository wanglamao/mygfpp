...
def cal_lap_loss(in_tensor):
    ...
    in_tensor = torch.cat(
        [in_tensor[:, :, 0:1], in_tensor, in_tensor[:, :, -1:]], dim=-1
    )
    lap_kernel = (
        torch.Tensor((-0.5, 1.0, -0.5)).reshape([1, 1, 3]).float().to(in_tensor.device)
    )  # [1, 1, kw]
    loss_lap = 0
...
def fit_3dmm_for_a_video(
    ...
    if nerf:  # single video
        lm_name = video_name.replace("/raw/", "/processed/").replace(
            ".mp4", "/lms_2d.npy"
        )
    else:
    ...
    if os.path.exists(lm_name):
        ...
        try:
            ...
            img_lm478, vid_lm478 = landmarker.extract_lm478_from_frames(
                frames, anti_smooth_factor=20
            )
        ...
        if lms is None:
            print(
                f"get None lms_2d, please check whether each frame has one head, exiting... {lm_name}"
            )
            return False
    lms = lms[:, :468, :]
    ...
    if keypoint_mode == "mediapipe":
        ...
        if nerf:  # single video
            out_name = video_name.replace("/raw/", "/processed/").replace(
                ".mp4", "/coeff_fit_mp.npy"
            )
        else:
            out_name = video_name.replace("/video/", "/coeff_fit_mp/").replace(
                ".mp4", "_coeff_fit_mp.npy"
            )
    else:
        ...
        if nerf:  # single video
            out_name = video_name.replace("/raw/", "/processed/").replace(
                ".mp4", "_coeff_fit_lm68.npy"
            )
        else:
            out_name = video_name.replace("/video/", "/coeff_fit_lm68/").replace(
                ".mp4", "_coeff_fit_lm68.npy"
            )
    try:
    ...
    if id_mode == "global":
        ...
        raise NotImplementedError(
            f"id mode {id_mode} not supported! we only support global or finegrained."
        )
    ...
    for _ in range(200):
        ...
        loss_lap = (
            cal_lap_loss(id_para)
            + cal_lap_loss(exp_para)
            + cal_lap_loss(euler_angle) * 0.3
            + cal_lap_loss(trans) * 0.3
        )
        ...
        loss = (
            loss_lan
            + loss_regid * LAMBDA_REG_ID
            + loss_regexp * LAMBDA_REG_EXP
            + loss_vel_id * LAMBDA_REG_VEL_ID
            + loss_vel_exp * LAMBDA_REG_VEL_EXP
            + loss_lap * LAMBDA_REG_LAP
        )
    ...
    for i in range(int((num_frames - 1) / batch_size + 1)):
        ...
        for j in range(50):
            ...
            # loss_lap = cal_lap_loss(proj_geo)
            loss_lap = (
                cal_lap_loss(sel_id_para)
                + cal_lap_loss(sel_exp_para)
                + cal_lap_loss(sel_euler_angle) * 0.3
                + cal_lap_loss(sel_trans) * 0.3
            )
    ...
    if debug:
        import imageio
        from gfpp.utils.visualization.vis_cam3d.camera_pose_visualizer import (
            CameraPoseVisualizer,
        )
        from data_util.face3d_helper import Face3DHelper
        from gfpp.data_gen.utils.process_video.extract_blink import get_eye_area_percent
        ...
        idexp_lm3d = face3d_helper.reconstruct_idexp_lm3d_np(
            coeff_dict["id"], coeff_dict["exp"]
        ).reshape([t, -1])
        cano_lm3d = (
            idexp_lm3d / 10
            + face3d_helper.key_mean_shape.squeeze().reshape([1, -1]).cpu().numpy()
        )
        ...
        xy_camera_visualizer = CameraPoseVisualizer(
            xlim=[
                extrinsic[:, 0, 3].min().item() - 0.5,
                extrinsic[:, 0, 3].max().item() + 0.5,
            ],
            ylim=[
                extrinsic[:, 1, 3].min().item() - 0.5,
                extrinsic[:, 1, 3].max().item() + 0.5,
            ],
            zlim=[
                extrinsic[:, 2, 3].min().item() - 0.5,
                extrinsic[:, 2, 3].max().item() + 0.5,
            ],
            view_mode="xy",
        )
        ...
        if nerf:
            debug_name = video_name.replace("/raw/", "/processed/").replace(
                ".mp4", "/debug_fit_3dmm.mp4"
            )
        else:
            debug_name = video_name.replace("/video/", "/coeff_fit_debug/").replace(
                ".mp4", "_debug.mp4"
            )
        try:
        ...
        for i in tqdm.trange(
            min(250, len(frames)), desc=f"rendering debug video to {debug_name}.."
        ):
            xy_cam3d_img = xy_camera_visualizer.extrinsic2pyramid(
                extrinsic[i], focal_len_scaled=0.25
            )
            xy_cam3d_img = cv2.resize(xy_cam3d_img, (512, 512))
            xz_cam3d_img = xz_camera_visualizer.extrinsic2pyramid(
                extrinsic[i], focal_len_scaled=0.25
            )
            ...
            for gt_lm in lms[i]:
                img2 = cv2.circle(
                    img2, gt_lm.cpu().numpy().astype(int), 2, gt_lm_color, thickness=1
                )
            ...
            for j in range(len(cano_lm3d[i])):
                x, y, _ = cano_lm3d[i, j]
                color = (255, 0, 0)
                cano_lm3d_img = cv2.circle(
                    cano_lm3d_img, center=(x, y), radius=3, color=color, thickness=-1
                )
            cano_lm3d_img = cv2.flip(cano_lm3d_img, 0)

            _, secc_img = secc_renderer(
                id_para[0:1],
                exp_para[i : i + 1],
                euler_angle[i : i + 1] * 0,
                trans[i : i + 1] * 0,
            ...
            out_img2 = np.concatenate(
                [xy_cam3d_img, xz_cam3d_img, cano_lm3d_img], axis=1
            ).astype(np.uint8)
...
def out_exist_job(vid_name):
    out_name = vid_name.replace("/video/", "/coeff_fit_mp/").replace(
        ".mp4", "_coeff_fit_mp.npy"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ds_name", default="nerf"
    )  # 'nerf' | 'CelebV-HQ' | 'TH1KH_512' | etc
    ...
    parser.add_argument(
        "--large_yaw_threshold", default=9999999.9, type=float
    )  # could be 0.7
    ...
    if ds_name.lower() == "nerf":  # 处理单个视频
        vid_names = [vid_dir]
        out_names = [
            video_name.replace("/raw/", "/processed/").replace(
                ".mp4", "_coeff_fit_mp.npy"
            )
        ...
        out_names = [
            video_name.replace("/video/", "/coeff_fit_mp/").replace(
                ".mp4", "_coeff_fit_mp.npy"
            )
            for video_name in vid_names
        ]
    ...
    failed_img_names = []
    for i in tqdm.trange(
        len(vid_names), desc=f"process {process_id}: fitting 3dmm ..."
    ):
        img_name = vid_names[i]
        try:
            is_person_specific_data = ds_name == "nerf"
            success = fit_3dmm_for_a_video(
                img_name,
