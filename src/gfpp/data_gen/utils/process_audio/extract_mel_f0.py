...
def extract_f0_from_wav_and_mel(
    wav,
    mel,
    hop_size=320,
    ...
    f0_max = 750
    f0 = (
        parselmouth.Sound(wav, audio_sample_rate)
        .to_pitch_ac(
...
def extract_mel_f0_from_fname(wav_name=None, out_name=None):
    try:
        out_name = wav_name.replace(".wav", "_mel_f0.npy").replace(
            "/audio/", "/mel_f0/"
        )
...
def extract_mel_f0_from_video_name(mp4_name, wav_name=None, out_name=None):
    if mp4_name.endswith(".mp4"):
        ...
        if out_name is None:
            out_name = mp4_name.replace(".mp4", "_mel_f0.npy").replace(
                "/video/", "/mel_f0/"
            )
    elif mp4_name.endswith(".wav"):
        wav_name = mp4_name
        if out_name is None:
            out_name = mp4_name.replace(".wav", "_mel_f0.npy").replace(
                "/audio/", "/mel_f0/"
            )
