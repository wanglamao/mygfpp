...
def load_img_to_normalized_512_bchw_tensor(img_name):
    ...
    img = (
        ((torch.tensor(img) - 127.5) / 127.5).float().unsqueeze(0).permute(0, 3, 1, 2)
    )  # [b,c,h,w]
    return img
...
def smooth_features_xd(in_tensor, kernel_size=7):
    ...
    in_tensor = torch.cat(
        [
            torch.flip(in_tensor[0:pad], dims=[0]),
            in_tensor,
            torch.flip(in_tensor[t - pad : t], dims=[0]),
        ],
        dim=0,
    )
    if ndim == 2:  # tc
        ...
        in_tensor = in_tensor.permute(1, 2, 3, 0).reshape(
            [-1, 1, t + 2 * pad]
        )  # [c, 1, t]
    elif ndim == 5:  # tcchw, like deformation
        _, c1, c2, h, w = in_tensor.shape
        in_tensor = in_tensor.permute(1, 2, 3, 4, 0).reshape(
            [-1, 1, t + 2 * pad]
        )  # [c, 1, t]
    else:
...
def extract_audio_motion_from_ref_video(video_name):
    ...
    def get_f0(wav16k_name):
        from gfpp.data_gen.process_lrs3.process_audio_mel_f0 import (
    ...
    def get_exp(video_name):
        from gfpp.data_gen.utils.process_video.fit_3dmm_landmark import (
