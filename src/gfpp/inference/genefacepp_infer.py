...
# common utils
from gfpp.utils.commons.hparams import hparams, set_hparams
from gfpp.utils.commons.tensor_utils import (
    # move_to_cuda,
    convert_to_np,
    convert_to_tensor,
)
from gfpp.utils.commons.ckpt_utils import load_ckpt

# 3DMM-related utils
# from gfpp.deep_3drecon.deep_3drecon_models.bfm import ParametricFaceModel
from gfpp.data_util.face3d_helper import Face3DHelper
from gfpp.deep_3drecon.secc_renderer import SECC_Renderer

# from gfpp.data_gen.eg3d.convert_to_eg3d_convention import (
#     get_eg3d_convention_camera_pose_intrinsic,
# )
from gfpp.data_gen.utils.mp_feature_extractors.face_landmarker import (
    index_lm68_from_lm478,
    index_lm131_from_lm478,
)

# Face Parsing
from gfpp.data_gen.utils.mp_feature_extractors.mp_segmenter import MediapipeSegmenter

# from gfpp.data_gen.utils.process_video.extract_segment_imgs import (
#     inpaint_torso_job,
#     extract_background,
# )

# other inference utils
from gfpp.inference.infer_utils import (
    mirror_index,
    # load_img_to_512_hwc_array,
    # load_img_to_normalized_512_bchw_tensor,
)
from gfpp.inference.infer_utils import smooth_features_xd
from gfpp.utils.commons.pitch_utils import f0_to_coarse

# Dataset Related
from gfpp.tasks.radnerfs.dataset_utils import (
    RADNeRFDataset,
    # get_boundary_mask,
    # dilate_boundary_mask,
    # get_lf_boundary_mask,
)

# Method Related
from gfpp.modules.audio2motion.vae import VAEModel, PitchContourVAEModel
from gfpp.modules.postnet.lle import compute_LLE_projection, find_k_nearest_neighbors
from gfpp.modules.radnerfs.utils import (
    get_audio_features,
    get_rays,
    get_bg_coords,
    convert_poses,
...
from gfpp.modules.radnerfs.radnerf import RADNeRF
from gfpp.modules.radnerfs.radnerf_sr import RADNeRFwithSR
from gfpp.modules.radnerfs.radnerf_torso import RADNeRFTorso
from gfpp.modules.radnerfs.radnerf_torso_sr import RADNeRFTorsowithSR
...
def vis_cano_lm3d_to_imgs(cano_lm3d, hw=512):
    ...
    for i_img in range(len(cano_lm3d)):
        ...
        for i in range(len(lm2d)):
            ...
            y = WH - y
            img = cv2.putText(
                img, f"{i}", org=(x, y), fontFace=font, fontScale=0.3, color=(255, 0, 0)
            )
        frame_lst.append(img)
    return frame_lst


def inject_blink_to_lm68(
    lm68, opened_eye_area_percent=0.6, closed_eye_area_percent=0.15
):
    # [T, 68, 2]
    # lm68[:,36:48] = lm68[0:1,36:48].repeat([len(lm68), 1, 1])
    opened_eye_lm68 = copy.deepcopy(lm68)
    eye_area_percent = 0.6 * torch.ones(
        [len(lm68), 1], dtype=opened_eye_lm68.dtype, device=opened_eye_lm68.device
    )

    eye_open_scale = (
        (opened_eye_lm68[:, 41, 1] - opened_eye_lm68[:, 37, 1])
    ...
    closed_eye_lm68[:, 37] = closed_eye_lm68[:, 41] = (
        closed_eye_lm68[:, 36] * 0.67 + closed_eye_lm68[:, 39] * 0.33
    )
    closed_eye_lm68[:, 38] = closed_eye_lm68[:, 40] = (
        closed_eye_lm68[:, 36] * 0.33 + closed_eye_lm68[:, 39] * 0.67
    )
    closed_eye_lm68[:, 43] = closed_eye_lm68[:, 47] = (
        closed_eye_lm68[:, 42] * 0.67 + closed_eye_lm68[:, 45] * 0.33
    )
    closed_eye_lm68[:, 44] = closed_eye_lm68[:, 46] = (
        closed_eye_lm68[:, 42] * 0.33 + closed_eye_lm68[:, 45] * 0.67
    )
    ...
    for i in range(T):
        if (i + 25) % period == 0:
            for j in range(dur):
                ...
                lm68[idx, 36:48] = (
                    lm68[idx, 36:48] * (1 - blink_factor)
                    + closed_eye_lm68[idx, 36:48] * blink_factor
                )
                eye_area_percent[idx] = (
                    opened_eye_area_percent * (1 - blink_factor)
                    + closed_eye_area_percent * blink_factor
                )
...
class GeneFace2Infer:
    def __init__(
        self, audio2secc_dir, postnet_dir, head_model_dir, torso_model_dir, device=None
    ):
        if device is None:
    ...
    def load_audio2secc(self, audio2secc_dir):
        ...
        if hparams.get("use_pitch", False) is True:
            self.model = PitchContourVAEModel(
                hparams, in_out_dim=self.in_out_dim, audio_in_dim=audio_in_dim
            )
        else:
    ...
    def load_postnet(self, postnet_dir):
        if postnet_dir == "":
            return None
        from gfpp.modules.postnet.models import PitchContourCNNPostNet

        set_hparams(
            f"{os.path.dirname(postnet_dir) if os.path.isfile(postnet_dir) else postnet_dir}/config.yaml"
        )
        ...
        load_ckpt(
            self.model, f"{postnet_dir}", steps=20000, model_name="model", strict=True
        )
        return self.model
    ...
    def infer_once(self, inp, output_dir):
        self.inp = inp
        samples = self.prepare_batch_from_inp(inp)
        out_name = self.forward_system(samples, inp, output_dir)
        return out_name
    ...
    def prepare_batch_from_inp(self, inp):
        ...
        sample["id"] = (
            torch.tensor(self.dataset.ds_dict["id"][0:1]).cuda().repeat([t_x // 2, 1])
        )
        pose_lst = []
        euler_lst = []
        trans_lst = []
        ...
        if "-" in inp["drv_pose"]:
            ...
            ds_euler_lst = [
                torch.tensor(self.dataset.ds_dict["euler"][i])
                for i in range(start_idx, end_idx)
            ]
            ds_trans_lst = [
                torch.tensor(self.dataset.ds_dict["trans"][i])
                for i in range(start_idx, end_idx)
            ]
    ...
    def get_hubert(self, wav16k_name):
        from gfpp.data_gen.utils.process_audio.extract_hubert import (
            get_hubert_from_16k_wav,
        )
    ...
    def get_f0(self, wav16k_name):
        from gfpp.data_gen.utils.process_audio.extract_mel_f0 import (
    ...
    def forward_audio2secc(self, batch, inp=None):
        ...
        pred = self.audio2secc_model.forward(
            batch, ret=ret, train=False, temperature=inp["temperature"]
        )
        if pred.shape[-1] == 144:
            id = ret["pred"][0][:, :80]
            exp = ret["pred"][0][:, 80:]
        ...
        if inp["debug"]:
            with torch.no_grad():
                ...
                num_iters = (
                    len(id) // chunk_size
                    if len(id) % chunk_size == 0
                    else len(id) // chunk_size + 1
                )
            ...
            _, src_secc_color = self.secc_renderer(
                id[0:1], exp[0:1], zero_eulers[0:1], zero_trans[0:1]
            )
            _, cano_secc_color = self.secc_renderer(
                id[0:1] * 0, exp[0:1] * 0, zero_eulers[0:1], zero_trans[0:1]
            )
        ...
        if hparams.get("normalize_cond", True):
            idexp_lm3d_ds_normalized = (
                idexp_lm3d_ds - idexp_lm3d_mean
            ) / idexp_lm3d_std
        else:
        ...
        if self.postnet_model is not None:
            ...
            f0 = self.audio2secc_model.downsampler(batch["f0"].unsqueeze(-1)).squeeze(
                -1
            )
            pitch = self.audio2secc_model.pitch_embed(f0_to_coarse(f0))
            b, t, *_ = pitch.shape
            raw_pred_lm3d = self.face3d_helper.reconstruct_idexp_lm3d(id, exp).reshape(
                [t, -1, 3]
            )
            raw_pred_lm3d = raw_pred_lm3d[:, index_lm68_from_lm478].reshape(
                [1, t, 68 * 3]
            )

            idexp_lm3d = self.postnet_model(
                raw_pred_lm3d, pitch[:, : exp.shape[0] * 2, :]
            ).reshape([t, 68 * 3])

            idexp_lm3d_ds_lle = idexp_lm3d_ds[:, index_lm68_from_lm478].reshape(
                [-1, 68 * 3]
            )
            feat_fuse, _, _ = compute_LLE_projection(
                feats=idexp_lm3d[:, : 68 * 3],
                feat_database=idexp_lm3d_ds_lle[:, : 68 * 3],
                K=10,
            )
            # feat_fuse = smooth_features_xd(feat_fuse, kernel_size=3)
            idexp_lm3d[:, : 68 * 3] = (
                LLE_percent * feat_fuse + (1 - LLE_percent) * idexp_lm3d[:, : 68 * 3]
            )
            idexp_lm3d = idexp_lm3d.reshape([t, 68, 3])
            idexp_lm3d_normalized = (idexp_lm3d - idexp_lm3d_mean) / idexp_lm3d_std
            idexp_lm3d_normalized = torch.clamp(
                idexp_lm3d_normalized, min=lower, max=upper
            )
        ...
        cano_lm3d = (
            idexp_lm3d_mean + idexp_lm3d_std * idexp_lm3d_normalized
        ) / 10 + self.face3d_helper.key_mean_shape[index_lm68_from_lm478].unsqueeze(0)
        eye_area_percent = self.opened_eye_area_percent * torch.ones(
            [len(cano_lm3d), 1], dtype=cano_lm3d.dtype, device=cano_lm3d.device
        )

        if inp["blink_mode"] == "period":
            cano_lm3d, eye_area_percent = inject_blink_to_lm68(
                cano_lm3d, self.opened_eye_area_percent, self.closed_eye_area_percent
            )
            print("Injected blink to idexp_lm3d by directly editting.")
        batch["eye_area_percent"] = eye_area_percent
        idexp_lm3d_normalized = (
            (
                cano_lm3d
                - self.face3d_helper.key_mean_shape[index_lm68_from_lm478].unsqueeze(0)
            )
            * 10
            - idexp_lm3d_mean
        ) / idexp_lm3d_std
        ...
        idexp_lm3d_normalized_ = (
            idexp_lm3d_normalized[0:1, :].repeat([len(exp), 1, 1]).clone()
        )
        ...
        cond_wins = [
            get_audio_features(cond_win, att_mode=2, index=idx)
            for idx in range(len(cond_win))
        ]
    ...
    def forward_secc2video(self, batch, inp=None, output_dir=None):
        ...
        if inp["low_memory_usage"]:
            ...
            if inp["debug"]:
                ...
                cano_lm3d_frames = (
                    convert_to_tensor(np.stack(cano_lm3d_frame_lst)).permute(0, 3, 1, 2)
                    / 127.5
                    - 1
                )
                imgs = torch.cat(
                    [pred_rgbs, cano_lm3d_frames, secc_img], dim=3
                )  # [B, C, H, W]
            else:
                imgs = pred_rgbs
            imgs = imgs.clamp(-1, 1)
            ...
            for i in tqdm.trange(
                len(out_imgs),
                desc=f"ImageIO is saving video using FFMPEG(h264) to {tmp_out_name}",
            ):
                writer.append_data(out_imgs[i])
            writer.close()

        save_file_name = os.path.join(output_dir, "output.mp4")
        cmd = f"ffmpeg -i {tmp_out_name} -i {self.wav16k_name} -y -shortest -c:v libx264 -y -shortest {save_file_name}"
        # cmd = f"ffmpeg -i {tmp_out_name} -i {self.wav16k_name} -y -shortest -c:v libx264 -pix_fmt yuv420p -b:v 6000k -y -v quiet -shortest {inp['out_name']}"
        ret = os.system(cmd)
        if ret == 0:
    ...
    @torch.no_grad()
    def forward_system(self, batch, inp, output_dir):
        self.forward_audio2secc(batch, inp)
        return self.forward_secc2video(batch, inp, output_dir)
    ...
    def example_run(cls, inp=None):
        ...
        infer_instance = cls(
            inp["a2m_ckpt"], inp["postnet_ckpt"], inp["head_ckpt"], inp["torso_ckpt"]
        )
        infer_instance.infer_once(inp, output_dir="./")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--a2m_ckpt", default="checkpoints/audio2motion_vae"
    )  # checkpoints/0727_audio2secc/audio2secc_withlm2d100_randomframe
    parser.add_argument("--head_ckpt", default="")
    parser.add_argument("--postnet_ckpt", default="")
    parser.add_argument("--torso_ckpt", default=None)
    ...
    parser.add_argument(
        "--raymarching_end_threshold", default=0.01, help="increase it to improve fps"
    )  # nearest | random
