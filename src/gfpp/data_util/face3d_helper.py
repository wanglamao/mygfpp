...
from gfpp.deep_3drecon.deep_3drecon_models.bfm import perspective_projection


class Face3DHelper(nn.Module):
    def __init__(
        self,
        bfm_dir="/mnt/nas/avatars/mygfpp/src/gfpp/deep_3drecon/BFM",
        keypoint_mode="lm68",
        use_gpu=True,
    ):
        super().__init__()
    ...
    def load_3dmm(self):
        ...
        self.register_buffer(
            "id_base", torch.from_numpy(model["idBase"]).float()
        )  # identity basis. [3*N,80], we have 80 eigen faces for identity
        self.register_buffer(
            "exp_base", torch.from_numpy(model["exBase"]).float()
        )  # expression basis. [3*N,64], we have 64 eigen faces for expression

        self.register_buffer(
            "mean_texure", torch.from_numpy(model["meantex"].transpose()).float()
        )  # mean face texture. [3*N,1] (0-255)
        self.register_buffer(
            "tex_base", torch.from_numpy(model["texBase"]).float()
        )  # texture basis. [3*N,80], rgb=3

        self.register_buffer(
            "point_buf", torch.from_numpy(model["point_buf"]).float()
        )  # triangle indices for each vertex that lies in. starts from 1. [N,8] (1-F)
        self.register_buffer(
            "face_buf", torch.from_numpy(model["tri"]).float()
        )  # vertex indices in each triangle. starts from 1. [F,3] (1-N)
        if self.keypoint_mode == "mediapipe":
            self.register_buffer(
                "key_points",
                torch.from_numpy(
                    np.load(
                        os.path.join(self.bfm_dir, "index_mp468_from_mesh35709.npy")
                    ).astype(np.int64)
                ),
            )
            ...
            self.register_buffer(
                "key_points",
                torch.from_numpy(model["keypoints"].squeeze().astype(np.int_)).long(),
            )  # vertex indices of 68 facial landmarks. starts from 1. [68,1]

        self.register_buffer(
            "key_mean_shape", self.mean_shape.reshape([-1, 3])[self.key_points, :]
        )
        self.register_buffer(
            "key_id_base",
            self.id_base.reshape([-1, 3, 80])[self.key_points, :, :].reshape([-1, 80]),
        )
        self.register_buffer(
            "key_exp_base",
        ...
        self.register_buffer(
            "persc_proj", torch.tensor(perspective_projection(focal=1015, center=112))
        )
    ...
    def reconstruct_face_mesh(self, id_coeff, exp_coeff):
        ...
        identity_diff_face = torch.matmul(
            id_coeff, id_base.transpose(0, 1)
        )  # [t,c],[c,3N] ==> [t,3N]
        expression_diff_face = torch.matmul(
            exp_coeff, exp_base.transpose(0, 1)
        )  # [t,c],[c,3N] ==> [t,3N]
    ...
    def reconstruct_cano_lm3d(self, id_coeff, exp_coeff):
        ...
        id_coeff = id_coeff.to(self.key_id_base.device)
        exp_coeff = exp_coeff.to(self.key_id_base.device)
        mean_face = self.key_mean_shape.squeeze().reshape(
            [1, -1]
        )  # [3*68, 1] ==> [1, 3*68]
    ...
    def reconstruct_idexp_lm3d_np(self, id_coeff, exp_coeff):
        ...
        expression_diff_face = np.dot(
            exp_coeff, exp_base.T
        )  # [t,c],[c,3*68] ==> [t,3*68]
    ...
    def close_mouth_for_idexp_lm3d(self, idexp_lm3d, freeze_as_first_frame=True):
        ...
        idexp_lm3d[:, 49:54, 1] = (
            idexp_lm3d[:, 49:54, 1] + idexp_lm3d[:, range(59, 54, -1), 1]
        ) / 2 + eps * 2
        ...
        idexp_lm3d[:, 61:64, 1] = (
            idexp_lm3d[:, 61:64, 1] + idexp_lm3d[:, range(67, 64, -1), 1]
        ) / 2 + eps
        idexp_lm3d[:, range(67, 64, -1), 1] = (
            idexp_lm3d[:, 61:64, 1] + idexp_lm3d[:, range(67, 64, -1), 1]
        ) / 2 - eps
        ...
        idexp_lm3d[:, 49:54, 1] += (
            (
                0.03
                - idexp_lm3d[:, 49:54, 1].mean(dim=1)
                + idexp_lm3d[:, 61:64, 1].mean(dim=1)
            )
            .unsqueeze(1)
            .repeat([1, 5])
        )
        idexp_lm3d[:, range(59, 54, -1), 1] += (
            (
                -0.03
                - idexp_lm3d[:, range(59, 54, -1), 1].mean(dim=1)
                + idexp_lm3d[:, range(67, 64, -1), 1].mean(dim=1)
            )
            .unsqueeze(1)
            .repeat([1, 5])
        )
    ...
    def close_eyes_for_idexp_lm3d(self, idexp_lm3d):
        ...
        eps = 0.003
        idexp_lm3d[:, 37:39, 1] = (
            idexp_lm3d[:, 37:39, 1] + idexp_lm3d[:, range(41, 39, -1), 1]
        ) / 2 + eps
        idexp_lm3d[:, range(41, 39, -1), 1] = (
            idexp_lm3d[:, 37:39, 1] + idexp_lm3d[:, range(41, 39, -1), 1]
        ) / 2 - eps

        idexp_lm3d[:, 43:45, 1] = (
            idexp_lm3d[:, 43:45, 1] + idexp_lm3d[:, range(47, 45, -1), 1]
        ) / 2 + eps
        idexp_lm3d[:, range(47, 45, -1), 1] = (
            idexp_lm3d[:, 43:45, 1] + idexp_lm3d[:, range(47, 45, -1), 1]
        ) / 2 - eps
...
if __name__ == "__main__":
    ...
    lm3d = face_mesh_helper.reconstruct_lm2d(
