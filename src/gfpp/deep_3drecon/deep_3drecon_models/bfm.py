...
# from gfpp.utils.commons.tensor_utils import convert_like
...
class ParametricFaceModel:
    def __init__(
        ...
        if keypoint_mode == "mediapipe":
            self.keypoints = np.load(
                os.path.join(bfm_folder, "index_mp468_from_mesh35709.npy")
            ).astype(np.int64)
        ...
        self.key_mean_shape = self.mean_shape.reshape([-1, 3])[
            self.keypoints, :
        ].reshape([-1, 3])
        self.key_id_base = self.id_base.reshape([-1, 3, 80])[self.keypoints, :].reshape(
            [-1, 80]
        )
        self.key_exp_base = self.exp_base.reshape([-1, 3, 64])[
            self.keypoints, :
        ].reshape([-1, 64])
    ...
    def to_camera(self, face_shape):
        face_shape[..., -1] = (
            self.camera_distance - face_shape[..., -1]
        )  # reverse the depth axis, add a fixed offset of length
        return face_shape
