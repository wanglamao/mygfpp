...
import os
from gfpp.deep_3drecon.util.mesh_renderer import MeshRenderer
from gfpp.deep_3drecon.deep_3drecon_models.bfm import ParametricFaceModel
from gfpp.deep_3drecon.constant import *
...
class SECC_Renderer(nn.Module):
    def __init__(self, rasterize_size=None, device="cuda"):
        ...
        self.fov = (
            2 * np.arctan(self.face_model.center / self.face_model.focal) * 180 / np.pi
        )
        self.znear = 5.0
        self.zfar = 15.0
        ...
        self.face_renderer = MeshRenderer(
            rasterize_fov=self.fov,
            znear=self.znear,
            zfar=self.zfar,
            rasterize_size=rasterize_size,
            use_opengl=False,
        ).cuda()
        face_feat = np.load(os.path.join(RECON_DIR, "ncc_code.npy"), allow_pickle=True)
        self.face_feat = torch.tensor(face_feat.T).unsqueeze(0).to(device=device)

        del_index_re = np.load(os.path.join(RECON_DIR, "bfm_right_eye_faces.npy"))
        del_index_re = del_index_re - 1
        del_index_le = np.load(os.path.join(RECON_DIR, "bfm_left_eye_faces.npy"))
    ...
    def forward(self, id, exp, euler, trans):
        ...
        face_mask, _, secc_face = self.face_renderer(
            face_vertex,
            self.face_buf.unsqueeze(0).repeat([bs, 1, 1]),
            feat=self.face_feat.repeat([bs, 1, 1]),
        )
...
if __name__ == "__main__":
    ...
    ret = np.load(
        "data/processed/videos/May/vid_coeff_fit.npy", allow_pickle=True
    ).tolist()
    idx = 6
    ...
    mask, secc = renderer(
        id, exp, angle * 0, trans * 0
    )  # [1, 1, 512, 512], [1, 3, 512, 512]
