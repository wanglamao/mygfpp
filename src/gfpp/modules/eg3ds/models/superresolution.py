...
from gfpp.modules.eg3ds.torch_utils import misc

from gfpp.modules.eg3ds.models.networks_stylegan2 import SynthesisBlock
from gfpp.modules.eg3ds.models.networks_stylegan3 import (
    SynthesisLayer as AFSynthesisLayer,
)
...
class LargeSynthesisBlock0(nn.Module):
    def __init__(self, channels, use_fp16, **block_kwargs):
        ...
        self.resblocks = nn.Sequential(
            *[
                ResBlock2d(256, kernel_size=3, padding=1)
                for _ in range(hparams["resblocks_in_large_sr"])
            ]
        )
...
class LargeSynthesisBlock1(nn.Module):
    def __init__(self, use_fp16, **block_kwargs):
        ...
        self.resblocks = nn.Sequential(
            *[
                ResBlock2d(128, kernel_size=3, padding=1)
                for _ in range(hparams["resblocks_in_large_sr"])
            ]
        )
...
class SuperresolutionHybrid8XDC(torch.nn.Module):
    def __init__(
        ...
        **block_kwargs,
        ...
        self.sr_antialias = sr_antialias
        if large_sr is True:
            self.block0 = LargeSynthesisBlock0(
                channels, use_fp16=sr_num_fp16_res > 0, **block_kwargs
            )
            self.block1 = LargeSynthesisBlock1(
                use_fp16=sr_num_fp16_res > 0, **block_kwargs
            )
        else:
            self.block0 = SynthesisBlock(
                channels,
