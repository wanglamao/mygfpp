...
from gfpp.modules.eg3ds.torch_utils.ops import conv2d_resample
from gfpp.modules.eg3ds.torch_utils.ops import upfirdn2d
from gfpp.modules.eg3ds.torch_utils.ops import bias_act
from gfpp.modules.eg3ds.torch_utils.ops import fma
...
def modulated_conv2d(
    ...
    if demodulate:
        dcoefs = (
            w.square().sum(dim=[2, 3, 4]) + 1e-8
        ).rsqrt()  # [NO] # [2, 512,512,3,3]==>[2, 512] 归一化
        ...
        if truncation_psi != 1:
            with torch.autograd.profiler.record_function("truncate"):
                ...
                if self.num_ws is None or truncation_cutoff is None:
                    ...
                    )  # 从w_avg出发向x前进，前进步数[0~1.]为truncation_psi
        return x
...
class Generator(torch.nn.Module):
    def __init__(
        ...
        **synthesis_kwargs,  # Arguments for SynthesisNetwork.
        ...
        if (
            hparams.get("gen_cond_mode", "none") == "mapping"
        ):  # comes from a attemp to inject landmark condition
            self.cond_dim = 204
            self.cond_mapping = MappingNetwork(
                z_dim=self.cond_dim,
                c_dim=0,
                w_dim=w_dim,
                num_ws=self.num_ws,
