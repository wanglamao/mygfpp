...
from gfpp.modules.eg3ds.torch_utils.ops import upfirdn2d
from gfpp.modules.eg3ds.models.networks_stylegan2 import (
    DiscriminatorBlock,
    MappingNetwork,
    DiscriminatorEpilogue,
)
from gfpp.modules.eg3ds.models.cond_encoder import LM3D_Win_Encoder
...
class SingleDiscriminator(torch.nn.Module):
    def __init__(
        ...
        if self.cond_dim > 0:
            ...
            c_dim += cond_out_dim
            self.cond_encoder = LM3D_Win_Encoder(
                self.cond_dim,
                hid_dim=hparams["cond_hid_dim"],
                out_dim=cond_out_dim,
                smo_size=hparams["smo_win_size"],
            )
        self.c_dim = c_dim
...
def filtered_resizing(image_orig_tensor, size, f, filter_mode="antialiased"):
    if filter_mode == "antialiased":
        ada_filtered_64 = torch.nn.functional.interpolate(
            image_orig_tensor,
            size=(size, size),
            mode="bilinear",
            align_corners=False,
        ...
        ada_filtered_64 = upfirdn2d.downsample2d(
            ada_filtered_64, f, down=2, flip_filter=True, padding=-1
        )
    elif filter_mode == "none":
        ada_filtered_64 = torch.nn.functional.interpolate(
            image_orig_tensor, size=(size, size), mode="bilinear", align_corners=False
        )
    elif type(filter_mode) == float:
        assert 0 < filter_mode < 1

        filtered = torch.nn.functional.interpolate(
            image_orig_tensor,
            size=(size, size),
            mode="bilinear",
            align_corners=False,
...
class DualDiscriminator(torch.nn.Module):
    def __init__(self):
        ...
        fp16_resolution = max(
            2 ** (self.img_resolution_log2 + 1 - self.num_fp16_res), 8
        )
        if cmap_dim is None:
            cmap_dim = channels_dict[4]
        if c_dim == 0:
    ...
    def forward(self, img, camera, cond=None, update_emas=False, **block_kwargs):
        image_raw = filtered_resizing(
            img["image_raw"], size=img["image"].shape[-1], f=self.resample_filter
        )
