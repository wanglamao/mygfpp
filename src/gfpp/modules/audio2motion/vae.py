...
import numpy as np
import copy
from gfpp.modules.audio2motion.flow_base import Glow, WN, ResidualCouplingBlock
from gfpp.modules.audio2motion.transformer_base import Embedding
    ...
    def __init__(
        ...
        self.pre_net = nn.Sequential(
            *[
                (
                    nn.ConvTranspose1d(
                        latent_channels, hidden_channels, kernel_size=s, stride=s
                    )
                    if i == 0
                    else nn.ConvTranspose1d(
                        hidden_channels, hidden_channels, kernel_size=s, stride=s
                    )
                )
                for i, s in enumerate(strides)
            ]
...
class FVAE(nn.Module):
    ...
    def forward(
        self, x=None, x_mask=None, g=None, infer=False, temperature=1.0, **kwargs
    ):
        """
        ...
        if not infer:
            ...
            if self.sqz_prior:
                ...
                if self.use_pos_embed:
                    position = self.embed_positions(
                        z.transpose(1, 2).abs().sum(-1)
                    ).transpose(1, 2)
                    z = z + position
                q = self.query_proj(
                    z.mean(dim=-1, keepdim=True).transpose(1, 2)
                )  # [B, 1, C=16]
                ...
                style_encoding = out.repeat([1, z_q.shape[-1], 1]).transpose(
                    1, 2
                )  # [B, C=256, T]
                ...
                if self.in_out_channels == 71:
                    x_recon = torch.cat(
                        [
                            self.exp_decoder(z_q, x_mask, g),
                            self.pose_decoder(z_q, x_mask, g),
                        ],
                        dim=1,
                    )
                else:
            ...
            return (
                ...
                z_p.transpose(1, 2),
                m_q.transpose(1, 2),
                logs_q.transpose(1, 2),
            )
        else:
            latent_shape = [g_sqz.shape[0], self.latent_size, g_sqz.shape[2]]
            z_p = (
                self.prior_dist.sample(latent_shape).to(g.device) * temperature
            )  # [B, latent_size, T_sqz]
            if self.use_prior_glow:
            ...
            if self.sqz_prior:
                ...
                attn = F.softmax(attn, dim=-1)
                out = torch.bmm(attn, v)  # [B, 1, C=256]
                style_encoding = out.repeat([1, z_p.shape[-1], 1]).transpose(
                    1, 2
                )  # [B, C=256, T]
                ...
                if self.in_out_channels == 71:
                    x_recon = torch.cat(
                        [self.exp_decoder(z_p, 1, g), self.pose_decoder(z_p, 1, g)],
                        dim=1,
                    )
                else:
...
class VAEModel(nn.Module):
    def __init__(
        ...
        self.vae = FVAE(
            in_out_channels=in_out_dim,
            hidden_channels=256,
            latent_size=16,
            kernel_size=5,
            enc_n_layers=8,
            dec_n_layers=4,
            gin_channels=cond_dim,
            strides=[
                4,
            ],
            use_prior_glow=self.use_prior_flow,
            glow_hidden=64,
            glow_kernel_size=3,
            glow_n_blocks=4,
            sqz_prior=sqz_prior,
        ...
        self.downsampler = LambdaLayer(
            lambda x: F.interpolate(
                x.transpose(1, 2), scale_factor=0.5, mode="linear"
            ).transpose(1, 2)
        )
    ...
    def forward(self, batch, ret, train=True, return_latent=False, temperature=1.0):
        infer = not train
        ...
        if not infer:
            ...
            x = exp
            x_recon, loss_kl, z_p, m_q, logs_q = self.vae(
                x=x, x_mask=mask, g=cond_feat, infer=False
            )
            ...
            x_recon, z_p = self.vae(
                x=None, x_mask=mask, g=cond_feat, infer=True, temperature=temperature
            )
            x_recon = x_recon * mask.unsqueeze(-1)
            ret["pred"] = x_recon
            ret["mask"] = mask
...
class PitchContourVAEModel(nn.Module):
    ...
    def forward(self, batch, ret, train=True, return_latent=False, temperature=1.0):
        ...
        if "blink" not in batch:
            batch["blink"] = torch.zeros(
                [f0.shape[0], f0.shape[1], 1], dtype=torch.long, device=f0.device
            )
        ...
        if hparams.get("use_mouth_amp_embed", False):
            mouth_amp = batch.get(
                "mouth_amp", torch.ones([f0.shape[0], 1], device=f0.device) * 0.4
            )
        ...
        if hparams.get("use_eye_amp_embed", False):
            eye_amp = batch.get(
                "eye_amp", torch.ones([f0.shape[0], 1], device=f0.device) * 0.4
            )
