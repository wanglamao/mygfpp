...
import numpy as np
from gfpp.modules.audio2motion.transformer_models import FFTBlocks
import gfpp.modules.audio2motion.utils as utils
from gfpp.modules.audio2motion.flow_base import Glow, WN, ResidualCouplingBlock
import torch.distributions as dist
from gfpp.modules.audio2motion.cnn_models import LambdaLayer, LayerNorm
...
class VQVAE(nn.Module):
    def __init__(
        ...
        self.vq = VectorQuantize(
            dim=hidden_channels, codebook_size=256, codebook_dim=16
        )
    ...
    def forward(self, batch, ret, log_dict=None, train=True):
        ...
        if not infer:
            ...
            if self.in_dim == 71:
                ...
                x = pose
            if self.enc_no_cond:
                x_recon, loss_commit, z_p, m_q, logs_q = self.vae(
                    x=x, x_mask=mask, g=torch.zeros_like(mel_feat), infer=False
                )
            else:
                x_recon, loss_commit, z_p, m_q, logs_q = self.vae(
                    x=x, x_mask=mask, g=mel_feat, infer=False
                )
