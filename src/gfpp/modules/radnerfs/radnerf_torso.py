...
class RADNeRFTorso(RADNeRF):
    ...
    def forward_torso(self, x, poses, c=None, image=None, weights_sum=None):
        ...
        if c is not None:
            h = torch.cat(
                [enc_x, enc_pose.repeat(x.shape[0], 1), c.repeat(x.shape[0], 1)], dim=-1
            )
        else:
        ...
        if mask.any():
            if hparams["torso_head_aware"]:
                if random.random() < 0.5:
                    ...
                    torso_alpha_mask, torso_color_mask, deform = self.forward_torso(
                        bg_coords[mask], poses, torso_individual_code, None, None
                    )
            else:
                torso_alpha_mask, torso_color_mask, deform = self.forward_torso(
                    bg_coords[mask], poses, torso_individual_code
                )
