...
from gfpp.utils.nn.seq_utils import expand_by_repeat_times
...
class NeRFRenderer(nn.Module):
    def __init__(self, hparams):
        ...
        if self.individual_embedding_dim > 0:
            self.individual_embeddings = nn.Parameter(
                torch.randn(
                    self.individual_embedding_num, self.individual_embedding_dim
                )
                * 0.1
            )
        ...
        )  # use 8 bit [0~255] to represent 8 points of a cube, if grid[i]>density threshold, set this bit to 1, so each cube can be represent as 0-255
        self.mean_density = 0
        self.iter_density = 0
    ...
    def update_extra_state(self, decay=0.95, S=128):
        ...
        if hparams.get("to_heatmap", False):
            ...
            lm478_px = convert_to_tensor(
                transform_normed_lm_to_pixel_lm(lm478)
            ).reshape([hparams["smo_win_size"], 1, 478, 3])
            heatmaps = []
            ...
            for ys in Y:
                for zs in Z:
                    ...
                    if hparams.get("to_heatmap", False) is True:
                        enc_a_ = enc_a[
                            torch.randint(low=0, high=len(enc_a), size=(len(xyzs),))
                        ]
                    ...
                    for cas in range(self.cascade):
                        ...
                        sigmas = (
                            self.density(cas_xyzs, enc_a)["sigma"]
                            .reshape(-1)
                            .detach()
                            .to(tmp_grid.dtype)
                        )
    ...
    def render(
        ...
        **kwargs
        ...
        if self.training:
            ...
            sigmas, rgbs, ambient = self.forward(
                xyzs, dirs, cond_feat, ind_code, cond_mask=cond_mask
            )
            ...
            while step < max_steps:
                ...
                if cond_mask is not None:
                    sigmas, rgbs, ambient = self.forward(
                        xyzs, dirs, cond_feat, ind_code, cond_mask=cond_mask[rays_alive]
                    )
                else:
                    sigmas, rgbs, ambient = self.forward(
                        xyzs, dirs, cond_feat, ind_code
                    )
