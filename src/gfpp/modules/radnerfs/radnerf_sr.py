...
from gfpp.modules.radnerfs.encoders.encoding import get_encoder
from gfpp.modules.radnerfs.renderer import NeRFRenderer
from gfpp.modules.radnerfs.cond_encoder import (
    AudioNet,
    AudioAttNet,
    MLP,
    HeatMapEncoder,
...
from gfpp.modules.radnerfs.utils import trunc_exp
from gfpp.modules.eg3ds.models.superresolution import *
...
class Superresolution(torch.nn.Module):
    ...
    def forward(self, rgb, **block_kwargs):
        x = rgb
        ws = torch.ones(
            [rgb.shape[0], 14, self.w_dim], dtype=rgb.dtype, device=rgb.device
        )
...
class RADNeRFwithSR(NeRFRenderer):
    def __init__(self, hparams):
        ...
        self.cond_prenet = AudioNet(
            self.cond_in_dim, self.cond_out_dim, win_size=self.cond_win_size
        )
        ...
        if self.with_att:
            self.cond_att_net = AudioAttNet(
                self.cond_out_dim, seq_len=self.smo_win_size
            )
        ...
        self.grid_interpolation_type = hparams[
            "grid_interpolation_type"
        ]  # smoothstep or linear
        self.position_embedder, self.position_embedding_dim = get_encoder(
            self.grid_type,
            input_dim=3,
            num_levels=16,
            level_dim=2,
            ...
            desired_resolution=hparams["desired_resolution"] * self.bound,
        ...
        self.ambient_net = MLP(
            self.position_embedding_dim + self.cond_out_dim,
            self.ambient_coord_dim,
            self.hidden_dim_ambient,
            self.num_layers_ambient,
        )
        # the learnable ambient grid
        self.ambient_embedder, self.ambient_embedding_dim = get_encoder(
            self.grid_type,
        ...
        self.sigma_net = MLP(
            self.position_embedding_dim + self.ambient_embedding_dim,
            1 + self.geo_feat_dim,
            self.hidden_dim_sigma,
        ...
        self.direction_embedder, self.direction_embedding_dim = get_encoder(
            "spherical_harmonics"
        )
        self.color_net = MLP(
            self.direction_embedding_dim
            + self.geo_feat_dim
            + self.individual_embedding_dim,
            3,
        ...
        self.lambda_ambient = torch.nn.Parameter(
    ...
    def cal_cond_feat(self, cond, eye_area_percent=None):
        ...
        if hparams.get("add_eye_blink_cond", False):
            ...
            blink_feat = self.blink_embedding(
                torch.tensor(0, device=cond_feat.device)
            ).reshape([1, -1])
            blink_feat = blink_feat * eye_area_percent.reshape([1, 1]).to(
                cond_feat.device
            )
            blink_feat = self.blink_encoder(blink_feat)
            cond_feat[..., : hparams["eye_blink_dim"]] = cond_feat[
                ..., : hparams["eye_blink_dim"]
            ] + blink_feat.expand(cond_feat[..., : hparams["eye_blink_dim"]].shape)
        if self.with_att:
    ...
    def forward(self, position, direction, cond_feat, individual_code, cond_mask=None):
        ...
        """
        cond_feat = cond_feat.repeat(
            [position.shape[0], 1]
        )  # [1,cond_dim] ==> [N, cond_dim]

        pos_feat = self.position_embedder(
            position, bound=self.bound
        )  # spatial feat f after E^3_{spatial} 3D grid in the paper

        # ambient
        ambient_inp = torch.cat(
            [pos_feat, cond_feat], dim=1
        )  # audio feat and spatial feat
        ambient_logit = self.ambient_net(
            ambient_inp
        ).float()  # the MLP after AFE in paper, use float(), prevent performance drop due to amp
        ambient_pos = torch.tanh(
            ambient_logit
        )  # normalized to [-1, 1], act as the coordinate in the 2D ambient tilegrid
        ambient_feat = self.ambient_embedder(
            ambient_pos, bound=1
        )  # E^2_{audio} in paper, 2D grid
        ...
        if individual_code is not None:
            color_inp = torch.cat(
                [
                    direction_feat,
                    geo_feat,
                    individual_code.repeat(position.shape[0], 1),
                ],
                dim=-1,
            )
        else:
