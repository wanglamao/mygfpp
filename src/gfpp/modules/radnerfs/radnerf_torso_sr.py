...
import gfpp.modules.radnerfs.raymarching as raymarching
...
from gfpp.modules.radnerfs.cond_encoder import AudioNet, AudioAttNet, MLP
from gfpp.modules.radnerfs.utils import trunc_exp
from gfpp.modules.radnerfs.utils import custom_meshgrid, convert_poses
from gfpp.modules.radnerfs.radnerf_sr import Superresolution
...
class RADNeRFTorsowithSR(RADNeRF):
    def __init__(self, hparams):
        ...
        if self.torso_individual_embedding_dim > 0:
            self.torso_individual_codes = nn.Parameter(
                torch.randn(
                    self.torso_individual_embedding_num,
                    self.torso_individual_embedding_dim,
                )
                * 0.1
            )

        # self.lm68_embedder = MLP(68*2, 16, 64, 3)
        self.lm68_embedder, self.lm68_embedding_dim = get_encoder(
            "frequency", input_dim=7 * 2, multires=4
        )

        self.torso_pose_embedder, self.pose_embedding_dim = get_encoder(
            "frequency", input_dim=6, multires=4
        )
        self.torso_deform_pos_embedder, self.torso_deform_pos_dim = get_encoder(
            "frequency", input_dim=2, multires=10
        )  # input 2D position
        ...
        deform_net_in_dim = (
            self.torso_deform_pos_dim
            + self.lm68_embedding_dim
            + self.torso_individual_embedding_dim
        )
        ...
        if hparams["torso_head_aware"]:
            if image is None:  # used in update_extra_states
                ...
                weights_sum = torch.zeros(
                    [x.shape[0], 1], dtype=h.dtype, device=h.device
                )
            head_color_weights_inp = torch.cat([image, weights_sum], dim=-1)
            head_color_weights_encoding = self.head_color_weights_encoder(
    ...
    def render(
        ...
        **kwargs
        ...
        with torch.no_grad():
            ...
            fars = fars.detach()
            # encode audio
            cond_feat = self.cal_cond_feat(
                cond, eye_area_percent=eye_area_percent
            )  # [1, 64]
            ...
            if self.training:
                ...
                weights_sum, ambient_sum, depth, image = (
                ...
                rays_alive = torch.arange(
                    n_alive, dtype=torch.int32, device=device
                )  # [N]
        ...
        if mask.any():
            if hparams["torso_head_aware"]:
                torso_alpha_mask, torso_color_mask, deform = self.forward_torso(
                    bg_coords[mask],
                    poses,
                    torso_individual_code,
                    image[mask],
                    weights_sum.unsqueeze(-1)[mask],
                    lm68=lm68,
                )
                ...
                torso_alpha_mask, torso_color_mask, deform = self.forward_torso(
                    bg_coords[mask], poses, torso_individual_code, lm68=lm68
                )
        ...
        results["torso_rgb_map"] = torso_bg_color.reshape([1, 256, 256, 3]).permute(
            0, 3, 1, 2
        )
    ...
    def update_extra_state(self, decay=0.95, S=128):
        ...
        lm68 = (
            self.lm68s[[rand_idx]].to(self.density_bitfield.device).reshape([1, 68 * 2])
        )
