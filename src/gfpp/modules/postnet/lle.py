...
def find_k_nearest_neighbors(feats, feat_database, K=10):
    ...
    distance_mat = (
        ...
        - 2 * feats @ feat_database.t()
    )  # [N_sample_in_batch, N_sample_in_database]
...
def solve_LLE_projection_batch(feat, feat_base):
    ...
    if K == 1:
        ...
        feat_fuse = torch.bmm(weights.unsqueeze(1), feat_base).squeeze(
            1
        )  # [N,1,K] @ [N,K,C] ==> [N,1,C] ==> [N, C]
        errors = (
            (torch.bmm(A, X.unsqueeze(-1)).squeeze() - B).abs().mean(dim=-1)
        )  # [N,]
...
def compute_LLE_projection(feats, feat_database, K=10):
    ...
    return:
        weights: [N_sample_K, ]
    """
    index_of_K_neighbors_in_database = find_k_nearest_neighbors(
        feats, feat_database, K
    )  # [N_sample_in_batch, K=10]
