...
from gfpp.utils.audio.pitch.extractor_utils import get_med_curve, clean_short_v_frag


def crepe_predict(
    audio, sr, model_capacity="full", center=True, step_size=10, verbose=1
):
...
def load_model(device, capacity="full"):
    ...
    # Load weights
    file = os.path.join(
        os.path.dirname(torchcrepe.__file__), "assets", f"{capacity}.pth"
    )
...
def crepe_predict_torch(
    ...
    warnings.filterwarnings(
        "ignore", message=r"Named tensors and all their associated APIs.*"
    )

    # Postprocessing breaks gradients, so just don't compute them
    with torch.no_grad():
        # Preprocess audio
        generator = preprocess(audio, sr, hop_length, batch_size, device, pad)
...
def frequency_to_cents(frequency):
    ...
    return 1200 * np.log2(frequency / 10.0 + 1e-8)
...
def f0_energy_corrector(wav_data_16k, f0_func, f0_min, f0_max, fix_octave_error=True):
    ...
    if fix_octave_error:
        for div, mul, thres in [
            ...
            energy_div_har, mask_div_har = get_energy_mask(
                lambda f0, m: f0 / div * m, mul, 3
            )
            ...
            for pos in div_pos:
                for s in range(10):
                    if pos - s not in div_pos and pos - s >= 0:
                        f0[pos - s] = pitches[
                            pos - s, np.abs(f0[pos] - pitches[pos - s]).argmin()
                        ]
    ...
    return (
        ...
        {
            "spec": spec_log,
            "energy_har": energy_har,
            "energy_halfhar": energy_mhalfhar,
            "r_energy": r_energy,
            "r_energy_2": r_energy_2,
            "mask_har": mask_har,
            "mask_halfhar": mask_mhalfhar,
...
def crepe_with_corrector(
    ...
    **kwargs,
    ...
    func_uv, func_f0, states = f0_energy_corrector(
        wav_data_16k, func_f0, f0_min, f0_max, fix_octave_error=True
    )
    ...
    try:
        ...
        f0_med_curve = interp1d(
            np.array(x_med_curve), np.array(y_med_curve), "nearest"
        )(np.arange(len(f0_10ms)))
    ...
    func_f0 = interp1d(x_10ms, f0_10ms, "nearest")
    func_uv, func_f0, states_ = f0_energy_corrector(
        wav_data_16k, func_f0, f0_min, f0_max, fix_octave_error=False
    )
