...
from gfpp.utils.audio.dct import dct
from gfpp.utils.audio.vad import trim_long_silences
...
def torch_wav2spec(
    wav, mel_basis, fft_size=1024, hop_size=256, win_length=1024, eps=1e-6
):
...
def librosa_linear2mel(linear_spec, hparams, num_mels=160, eps=1e-6):
    ...
    mel_basis = (
        torch.FloatTensor(mel_basis)
        .to(linear_spec.device)[None, :]
        .repeat(linear_spec.shape[0], 1, 1)
    )
