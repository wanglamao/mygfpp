...
from gfpp.utils.audio import librosa_wav2spec
from gfpp.utils.audio.io import save_wav

os.environ["TF_CPP_MIN_LOG_LEVEL"] = "2"
import librosa
import numpy as np
from scipy.interpolate import interp1d
from gfpp.utils.audio.pitch.crepe_utils import (
    crepe_with_corrector,
    crepe_predict,
    frequency_to_bins,
    crepe_predict_torch,
)
from gfpp.utils.audio.pitch.extractor_utils import (
    find_nearest_stft_bin,
    find_best_f0_using_har_energy,
)
...
def extract_pitch_simple(wav):
    ...
    n_mel_frames = (len(wav) + 1) // hparams["hop_size"] - hparams[
        "win_size"
    ] // hparams["hop_size"]
    return extract_pitch(
        hparams["pitch_extractor"],
        wav,
        hparams["hop_size"],
...
def extract_pitch(
    ...
    **kwargs,
    ...
    return get_pitch_extractor(extractor_name)(
        wav_data, hop_size, audio_sample_rate, f0_min, f0_max, **kwargs
    )
...
def reaper_extract_f0(
    ...
    **kwargs,
    ...
    with tempfile.TemporaryDirectory(dir=dirname) as _:
        ...
        if denoise:
            from gfpp.utils.audio import trim_long_silences

            wav_data_ = wav_data
            _, audio_mask, sr = trim_long_silences(
                wav_data, audio_sample_rate, vad_max_silence_length=20
            )
            ...
            subprocess.check_call(
                ...
                f"sox {new_fn}.wav {new_fn}.denoised.wav noisered {new_fn}-noise.prof 0.21; ",
                shell=True,
            )
        ...
        while retry > 0:
            ...
            try:
                f0 = reaper_extract_f0_(
                    f"{dirname}/2.wav", f"{dirname}/1.wav", dirname, f0_min, f0_max
                )[:-8]
                break
...
def reaper_extract_f0_(fwav1, fwav2, temp_dir, pitch_lower, pitch_upper):
    ...
    if not os.path.exists(straight_f0_file):
        subprocess.check_call(
            "utils/audio/pitch/bin/ExtractF0ByStraight frame_shift=%d "
            'min_f0=%d max_f0=%d wave="%s" output="%s"'
            % (frame_shift, pitch_lower, pitch_upper, fwav1, straight_f0_file),
            shell=True,
            timeout=20,
        )
    if use_reaper:
        ...
        if not os.path.exists(reaper_f0_file):
            subprocess.check_call(
                'utils/audio/pitch/bin/ReaperF0 wave="%s" output="%s" '
                "f0_min=%d f0_max=%d"
                % (fwav2, reaper_f0_file, pitch_lower, pitch_upper),
                shell=True,
                timeout=20,
        ...
        if not os.path.exists(interp_f0_file):
            subprocess.check_call(
                'utils/audio/pitch/bin/InterpF0 straight="%s" '
                'reaper="%s" output="%s"'
                % (straight_f0_file, reaper_f0_file, interp_f0_file),
                shell=True,
                timeout=20,
            )
