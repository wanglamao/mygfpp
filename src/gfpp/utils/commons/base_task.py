...
from gfpp.utils.commons.meters import AvgrageMeter
from gfpp.utils.commons.tensor_utils import tensors_to_scalars
from gfpp.utils.commons.trainer import Trainer
from gfpp.utils.nn.model_utils import print_arch, num_params
...
class BaseTask(nn.Module):
    ...
    def on_train_start(self):
        ...
        if torch.__version__.split(".")[0] == "2" and hparams.get(
            "torch_compile", False
        ):
    ...
    def training_step(self, sample, batch_idx, optimizer_idx=-1):
        ...
        for k, v in log_outputs.items():
            if "/" in k:
                ...
                assert (
                    len(k_split) == 2
                ), "we only support one `/` in tag_name, i.e., `<tag>/<sub_tag>`"
                k = k.replace("/", "_")
        ...
        if optimizer_idx >= 0:
            for params_group_i in range(
                len(self.trainer.optimizers[optimizer_idx].param_groups)
            ):
                log_outputs[
                    f"lr/optimizer{optimizer_idx}_params_group{params_group_i}"
                ] = self.trainer.optimizers[optimizer_idx].param_groups[params_group_i][
                    "lr"
                ]
        ...
        for k, v in log_outputs.items():
            if "/" in k:
                ...
                k = k.replace("/", "_")
            assert (
                k not in progress_bar_log
            ), f"we got duplicate tags in log_outputs, check this `{k}`"
    ...
    def start(cls):
        ...
        while is_port_in_use(int(os.environ["MASTER_PORT"])):
            print(
                f"| Port {os.environ['MASTER_PORT']} is in use. Change another port..."
            )
            os.environ["MASTER_PORT"] = str(random.randint(10000, 11000))
            time.sleep(1)
